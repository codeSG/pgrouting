...
``pgr_codesgDijkstra`` — Returns the shortest path(s) using Dijkstra algorithm.
...
.. index::
    single: codesgDijkstra(Minimal Use)
...
.. code-block:: none
    ...
    pgr_codesgDijkstra(edges_sql, start_vid, end_vid)
...
.. literalinclude:: doc-pgr_codesgDijkstra.queries
   :start-after: -- q1
   :end-before: -- q2


.. index::
    single: codesgDijkstra(Complete signature)
...
.. code-block:: none
    ...
    pgr_codesgDijkstra(edges_sql, start_vid, end_vid, directed);
    RETURNS SET OF (seq, path_seq, node, edge, cost, agg_cost) or EMPTY SET
