...
``pgr_dummyDijkstra`` — Returns the shortest path(s) using Dijkstra algorithm.
...
.. index::
    single: dummyDijkstra(Minimal Use)
...
.. code-block:: none
    ...
    pgr_dummyDijkstra(edges_sql, start_vid, end_vid)
...
.. literalinclude:: doc-pgr_dummyDijkstra.queries
   :start-after: -- q1
   :end-before: -- q2


.. index::
    single: dummyDijkstra(Complete signature)
...
.. code-block:: none
    ...
    pgr_dummyDijkstra(edges_sql, start_vid, end_vid, directed);
    RETURNS SET OF (seq, path_seq, node, edge, cost, agg_cost) or EMPTY SET
